...
pub fn exec(unit: SharedUnit) -> Result<u32, NuclErrors> {
    let (monitor, uid, name) = {
        let g = unit.lock()?;
        (g.get_autostart(), g.get_runas(), g.get_name().clone())
    };

    //for loggging:
    tracing::Span::current().record("unit_name", name.as_str());
    tracing::Span::current().record("uid", uid.get_uid());
    tracing::Span::current().record("gid", uid.get_gid());
    tracing::Span::current().record("monitor", monitor);

    debug!("Evaluating execution strategy");

    let pid = exec_process(unit.clone(), uid)?;

    info!(pid = pid, "Registering running unit");
    RunningRegistry::add_unit(unit, pid)?;
    Ok(pid)
}
fn exec_process(unit: SharedUnit, id: UserId) -> Result<u32, NuclErrors> {
